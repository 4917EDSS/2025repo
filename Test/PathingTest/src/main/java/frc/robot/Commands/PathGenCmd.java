  ...
  public ArrayList<int[]> getNeighbours(int[] pos, ArrayList<int[]> processed){
    ...
    for(int i=-1; i<2; i++){
      for(int j=-1; j<2; j++){
        ...
        if((pos[0]+i>=0&&pos[0]+i<=55&&pos[1]+j>=0&&pos[1]+j<=27&&field[pos[0]+i][pos[1]+j]==0&&(i!=0||j!=0))){
            boolean fail = false;
            for(int[] f : processed){
    ...
    while(!toSearch.isEmpty()) {
      currentPos = toSearch.get(0);
        ...
        for(int[] coord : toSearch) {
          if(calcF(coord)<calcF(currentPos) || Math.round((calcF(coord)*100000)) == Math.round(calcF(currentPos)*100000) && Math.round(calcH(coord)*100000) < Math.round(calcH(currentPos)*100000)) {
          ...
          if((currentPos[0])==(targetPos[0])&&(currentPos[1])==(targetPos[1])) {
            ...
            while(currentPathCoord[0] != startingPos[0]&&currentPathCoord[1] != startingPos[1]){
              ...
              currentPathCoord[0] = connections[tempCoord[0]][tempCoord[1]][0];
              currentPathCoord[1] = connections[tempCoord[0]][tempCoord[1]][1];
              count--;
              if(count<0){
          ...
          for(int[] neighbour : getNeighbours(currentPos, processed)) {
            ...
            if(!inToSearch || costToNeighbour < gVals[neighbour[0]][neighbour[1]]){
                  gVals[neighbour[0]][neighbour[1]] = costToNeighbour;
                    connections[currentPos[0]][currentPos[1]][0] = neighbour[0];
                    connections[currentPos[0]][currentPos[1]][1] = neighbour[1];
